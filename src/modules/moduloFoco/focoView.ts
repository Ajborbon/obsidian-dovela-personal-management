...
export class FocoView extends ItemView {
    ...
    private async drawView(activeFile: TFile): Promise<void> {
        ...
        try {
            ...
            // Debug: Verificar inmediatamente el DOM después de establecer innerHTML
            console.log('🔍 FOCO DEBUG: HTML establecido, verificando DOM inmediatamente...');
            console.log('🔍 FOCO DEBUG: Vista activa:', this.activeView);
            const immediateCheck = this.contentEl.querySelectorAll('.check-in-progress');
            const immediateCheckWithPath = this.contentEl.querySelectorAll('.check-in-progress[data-check-path]');
            const hierarchyItems = this.contentEl.querySelectorAll('.gtd-hierarchy-item');
            const hasInProgressItems = this.contentEl.querySelectorAll('.has-in-progress-tasks');
            console.log('🔍 FOCO DEBUG: Elementos .check-in-progress inmediatamente:', immediateCheck.length);
            console.log('🔍 FOCO DEBUG: Elementos .check-in-progress[data-check-path] inmediatamente:', immediateCheckWithPath.length);
            console.log('🔍 FOCO DEBUG: Elementos .gtd-hierarchy-item inmediatamente:', hierarchyItems.length);
            console.log('🔍 FOCO DEBUG: Elementos .has-in-progress-tasks inmediatamente:', hasInProgressItems.length);
            if (immediateCheck.length > 0) {
                console.log('🔍 FOCO DEBUG: Primer elemento .check-in-progress:', immediateCheck[0]);
                console.log('🔍 FOCO DEBUG: Dataset del primer elemento:', (immediateCheck[0] as HTMLElement).dataset);
            }
            if (hasInProgressItems.length > 0) {
                console.log('🔍 FOCO DEBUG: Elementos con has-in-progress-tasks (causando ⚡):', hasInProgressItems);
            }
    ...
    private async checkInProgressTasks(): Promise<void> {
        console.log('🔍 FOCO DEBUG: Iniciando checkInProgressTasks');
        const container = this.contentEl;
        const elementsToCheck = container.querySelectorAll('.check-in-progress[data-check-path]');
        console.log('🔍 FOCO DEBUG: Elementos encontrados con .check-in-progress[data-check-path]:', elementsToCheck.length);

        for (const element of Array.from(elementsToCheck)) {
            const htmlElement = element as HTMLElement;
            console.log('🔍 FOCO DEBUG: Elemento HTML:', htmlElement);
            console.log('🔍 FOCO DEBUG: Dataset completo:', htmlElement.dataset);
            console.log('🔍 FOCO DEBUG: data-check-path attribute:', htmlElement.getAttribute('data-check-path'));

            const filePath = htmlElement.dataset['checkPath'] || htmlElement.getAttribute('data-check-path');
            console.log('🔍 FOCO DEBUG: Procesando archivo:', filePath);
            ...
            if (filePath) {
                try {
                    ...
                    if (file && file.path === filePath) {
                        const content = await this.app.vault.read(file as any);
                        console.log('🔍 FOCO DEBUG: Contenido leído, longitud:', content.length);
                        console.log('🔍 FOCO DEBUG: Primeras 200 chars:', content.substring(0, 200));

                        const hasInProgressTask = /^- \[\/\]/m.test(content);
                        console.log('🔍 FOCO DEBUG: ¿Tiene tareas en progreso?', hasInProgressTask);

                        if (hasInProgressTask) {
                            htmlElement.classList.add('has-in-progress-tasks');
                            console.log('🔍 FOCO DEBUG: ✅ Clase has-in-progress-tasks agregada a:', filePath);
                        }
                    } else {
                        console.log('🔍 FOCO DEBUG: ❌ Archivo no encontrado o path no coincide:', filePath);
                    }
                } catch (error) {
                    console.warn('🔍 FOCO DEBUG: ❌ Error checking file for in-progress tasks:', filePath, error);
                }
            }
        }
        console.log('🔍 FOCO DEBUG: checkInProgressTasks completado');
    }
}
